const { Connection, PublicKey, Keypair, SystemProgram, LAMPORTS_PER_SOL } = require("@solana/web3.js");
const { AnchorProvider, Program, BN } = require("@coral-xyz/anchor");
const fs = require("fs");

const DEVNET_RPC = "https://devnet.helius-rpc.com/?api-key=f3417b56-61ad-4ba8-b0f9-3695ea859a58";
const PROGRAM_ID = new PublicKey("7qRu72wJ5AGcXkqnwXoNtkWt3Z6ZaJoyTQsEc5gzzkqK");

const IDL = JSON.parse(fs.readFileSync("src/lib/privatepoker_idl.json", "utf8"));

const keyPath = require("os").homedir() + "/.config/solana/id.json";
const keyData = JSON.parse(fs.readFileSync(keyPath, "utf8"));
const player1 = Keypair.fromSecretKey(Uint8Array.from(keyData));
const player2 = Keypair.generate();

const GAME_SEED = Buffer.from("poker_game");
const PLAYER_HAND_SEED = Buffer.from("player_hand");

function toLEBytes(value) {
  const bn = typeof value === "number" ? BigInt(value) : value;
  const bytes = new Uint8Array(8);
  let v = bn;
  for (let i = 0; i < 8; i++) {
    bytes[i] = Number(v & BigInt(0xff));
    v >>= BigInt(8);
  }
  return Buffer.from(bytes);
}

function getGamePDA(gameId) {
  const [pda] = PublicKey.findProgramAddressSync([GAME_SEED, toLEBytes(gameId)], PROGRAM_ID);
  return pda;
}

function getPlayerHandPDA(gameId, playerPubkey) {
  const [pda] = PublicKey.findProgramAddressSync([PLAYER_HAND_SEED, toLEBytes(gameId), playerPubkey.toBuffer()], PROGRAM_ID);
  return pda;
}

async function main() {
  const connection = new Connection(DEVNET_RPC, "confirmed");
  
  const provider = new AnchorProvider(connection, {
    publicKey: player1.publicKey,
    signTransaction: async (tx) => { 
      console.log(`\n🔑 signTransaction called for settle`);
      console.log(`   TX instructions: ${tx.instructions.length}`);
      tx.instructions.forEach((ix, idx) => {
        console.log(`   Instruction ${idx} program: ${ix.programId.toBase58()}`);
        console.log(`   Instruction ${idx} accounts (`);
        ix.keys.forEach((k, i) => {
          console.log(`     [${i}] ${k.pubkey.toBase58().substring(0, 20)}... - isSigner: ${k.isSigner}, isWritable: ${k.isWritable}`);
        });
        console.log(`   Instruction ${idx} signers:`, ix.keys.filter(k => k.isSigner).map(k => k.pubkey.toBase58().substring(0, 20)));
      });
      tx.sign(player1); 
      console.log(`   TX signatures after signing: ${tx.signatures.length}`);
      tx.signatures.forEach((sig, i) => {
        console.log(`     Sig ${i}: ${sig.publicKey.toBase58().substring(0, 20)}...`);
      });
      return tx; 
    },
    signAllTransactions: async (txs) => { 
      txs.forEach(tx => tx.sign(player1)); 
      return txs; 
    },
  }, { commitment: "confirmed" });
  
  const program = new Program(IDL, provider);
  
  const gameId = Math.floor(Math.random() * 1_000_000_000);
  const gamePDA = getGamePDA(gameId);
  const hand1PDA = getPlayerHandPDA(gameId, player1.publicKey);
  const buyIn = 0.05 * LAMPORTS_PER_SOL;
  
  try {
    console.log("Creating game...");
    const createTx = await program.methods
      .createGame(new BN(gameId), new BN(buyIn))
      .accounts({
        game: gamePDA,
        playerHand: hand1PDA,
        player1: player1.publicKey,
        systemProgram: SystemProgram.programId,
      })
      .rpc();
    console.log("✅ Game created:", createTx);
    
    // Verify game account exists and is owned by program
    const gameAccount = await connection.getAccountInfo(gamePDA);
    console.log("\n📊 Game PDA Info:");
    console.log(`   Address: ${gamePDA.toBase58()}`);
    console.log(`   Exists: ${gameAccount !== null}`);
    console.log(`   Owner: ${gameAccount?.owner.toBase58()}`);
    console.log(`   Expected owner: ${PROGRAM_ID.toBase58()}`);
    console.log(`   Owner matches: ${gameAccount?.owner.equals(PROGRAM_ID)}`);
    
    if (gameAccount?.owner.equals(PROGRAM_ID)) {
    console.log("\n🏆 Now settling game...");
    const settleTx = await program.methods
      .settleGame(0, new BN(buyIn))
      .accounts({
        game: gamePDA,
        winner: player1.publicKey,
        loser: player2.publicKey,
        // Don't pass payer - let Anchor infer from provider
      })
      .rpc();      console.log("✅ Game settled!", settleTx);
    } else {
      console.log("\n❌ Game account not owned by program!");
    }
  } catch (err) {
    console.error("\n❌ Error:", err.message);
    if (err.logs) {
      console.error("\nProgram Logs:");
      err.logs.forEach(l => console.error(`  ${l}`));
    }
  }
}

main().catch(console.error);
