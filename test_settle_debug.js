const { Connection, PublicKey, Keypair, SystemProgram, LAMPORTS_PER_SOL, Transaction, AccountMeta } = require("@solana/web3.js");
...
async function main() {
  ...
  const provider = new AnchorProvider(connection, {
    ...
    signTransaction: async (tx) => {
      console.log("\\n🔑 BEFORE signing:");
      console.log(`   Transaction signatures: ${tx.signatures.length}`);
      console.log(`   Transaction feePayer: ${tx.feePayer?.toBase58()}`);
      console.log(`   Instruction signers:`);
      tx.instructions.forEach((ix, i) => {
        console.log(`\\n   Instruction ${i} accounts:`);
      ...
      tx.signatures.forEach((sig, i) => {
        console.log(`   Signature ${i} pubkey: ${sig.publicKey.toBase58()}`);
      });
